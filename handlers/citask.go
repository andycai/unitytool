...
// GetTask 获取任务详情
func GetTask(c *fiber.Ctx, db *gorm.DB) error {
	id := c.Params("id")
...
// RunTask 执行任务
func RunTask(c *fiber.Ctx, db *gorm.DB) error {
	id := c.Params("id")
	var task models.Task
	if err := db.First(&task, id).Error; err != nil {
		return c.Status(404).JSON(fiber.Map{
...
// GetTaskLogs 获取任务日志
func GetTaskLogs(c *fiber.Ctx, db *gorm.DB) error {
	taskID := c.Params("id")
...
func GetTaskStatus(c *fiber.Ctx, db *gorm.DB) error {
...
// executeTask 执行任务
func executeTask(task *models.Task, log *models.TaskLog, db *gorm.DB) {
	defer func() {
